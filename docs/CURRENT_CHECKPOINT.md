...
**Checkpoint date:** 27 September 2026
**Reason:** Root reviewed and integrated `API-PUBLIC-EVENT-LIST-CANDIDATE-CORE` from its dedicated task branch. The bounded reader uses existing current-public views, orders by immutable version-1 `published_at` descending and `event_id` ascending, and excludes withdrawn and non-live records. It is an internal candidate port only; no projection, filters, route, DTO, migration, or runtime binding was added. Root independently passed the focused WSL PGlite suite after integration (2/2). The implementation commit passed DB tests (16 files), full `npm test` (web 22, Worker 183, DB 16, evaluation 12), typecheck, build and diff checks; the later test-only follow-up passed the focused suite (2/2). Fixtures are authored fiction; hosted Neon and real event data remain unverified. The public-history projection remains read-only and requires moderator-reviewed metadata; withdrawn versions and all history after latest withdrawal remain hidden. MOD-01 and source/data rights still gate authenticated review writes and live history output. The GeoJSON demo route remains empty because no fixture has source-supported geometry.
...
The latest accepted task is `API-PUBLIC-EVENT-LIST-CANDIDATE-CORE`, integrated at root commits `5f14f2a` (reader), `4a0001e` (agent handoff) and `58d0a27` (acceptance tests), preserving agent commits `369694a`, `67f27da` and `96b4f4b`. It adds a bounded current-public live-event reader over existing safe views, with version-1 publication-time keyset ordering, exact cursor validation, parameterized limit-plus-one probing, and redacted failures. Root independently passed the focused WSL PGlite test (2/2); the task branch passed DB 16/16, full `npm test` (web 22, Worker 183, DB 16, evaluation 12), typecheck and build. No migration, grant, runtime configuration, public contract, filter semantics, route or database binding changed; hosted Neon behavior remains unverified. The latest history projection remains read-only and requires moderator-reviewed labels/summaries. Withdrawn versions and all history after latest withdrawal remain hidden. MOD-01 still owns authenticated review writes. UI-01 filters and UI-02 preferences remain accepted with no synthetic relevance matching.

ADR-018 records the inclusive CRS84 application query envelope `west=106.32, south=-6.40, east=106.98, north=-5.16`, outward-rounded from the Jakarta 2025–2029 regional-plan extents. It limits the GeoJSON request only; it is not an administrative boundary, event geometry, or warning area. The OpenAPI bbox description records it, and the accepted demo route validates against it. The implementation branch began at root checkpoint `3d04d5d` and is now on `main` at handoff `29aa6b4`; root acceptance is recorded in this checkpoint and the delivery log. No live DB reader, source-supported event geometry, or live spatial filtering is wired.

Local `main` includes root-reviewed GEO-STORE-CORE at merge `3207800`, L1-WRITE-IDEMPOTENCY-CORE at merge `59e59e8`, L1-FIXTURE-PIPE-CORE at merge `e5b1647`, EVAL-01-TOOLS at merge `9f80600`, API-PROJECT-CORE at integration commits `c383faa` and `249c5e2`, PUB-WRITE-CORE at merge `b2fb994`, L3-LEDGER-CORE at implementation `ed0f307` plus handoff `32d4678`, DB-TEST-RUNNER-ISOLATION at handoff `805ccd4`, L2-CONTEXT-PERSIST-CORE at merge `6effd54`, L2-CONTEXT-BRIDGE-CORE at merge `6ced3ea`, OBS-01-API-TELEMETRY-CORE at integration `8135a90`, OBS-01-L2-RETRIEVAL-TELEMETRY-CORE at integration `12509e3`, OBS-01-L3-LEDGER-TELEMETRY-CORE at integration `408de10`, RAG-CONTEXT-ASSEMBLY-CORE at handoff `f4ca0dc`, L2-DIRECT-REASONING-CORE at handoff `c3e2005`, L3-SINGLE-STEP-EXECUTOR-CORE at handoff `22b1f10` with root acceptance `d8a5b62`, API-GEOMETRY-CORE at handoff `c0d6ade`, API-DETAIL-HISTORY-ROUTES-CORE at handoff `48c7f73`, UI-API-DETAIL-HISTORY-CORE at handoff `e53caf5`, UI-02-LOCAL-PREFERENCES-CORE at handoff `b469a02`, and API-GEOJSON-ROUTE-CORE at handoff `29aa6b4`. The accepted project work includes BOOT-01, UI-00, UI-API-DETAIL-HISTORY-CORE, UI-02-LOCAL-PREFERENCES-CORE, PLATFORM-01, DATA-01, ING-PARSE-01, L2-ADAPTER-01, JOB-01, DATA-02-CORE, GEO-STORE-CORE, L1-WRITE-IDEMPOTENCY-CORE, L1-FIXTURE-PIPE-CORE, RAG-CORE, RAG-ACCESS-01, L2-CONTEXT-PERSIST-CORE, L2-CONTEXT-BRIDGE-CORE, RAG-CONTEXT-ASSEMBLY-CORE, L2-DIRECT-REASONING-CORE, L3-INSUFFICIENT-CONTEXT-ENTRY-CORE, L3-SINGLE-STEP-EXECUTOR-CORE, API-GEOMETRY-CORE, API-DETAIL-HISTORY-ROUTES-CORE, API-GEOJSON-ROUTE-CORE, PUB-POLICY-CORE, API-PROJECT-CORE, PUB-WRITE-CORE, L3-LEDGER-CORE, OBS-01-API-TELEMETRY-CORE, OBS-01-L2-RETRIEVAL-TELEMETRY-CORE, OBS-01-L3-LEDGER-TELEMETRY-CORE, DB-TEST-RUNNER-ISOLATION, and the synthetic-only casebook contract/validator. EVAL-01 real case collection remains planned because source/data rights are pending. Perry Tjahya and Jesaya Hamonangan Gaudensius Malau are identified as future independent reviewers; no human labels were created. Implemented fixtures remain synthetic, and historical/synthetic datasets cannot receive publishable policy dispositions.

The accepted work list now includes L1-FIXTURE-RUNNER-CORE, API-PUBLIC-LOOKUPS-CORE, API-PUBLIC-SNAPSHOT-CORE, API-PUBLIC-PROJECTION-SERVICE-CORE, API-PUBLIC-GEOMETRY-READER-CORE, API-PUBLIC-DETAIL-PROJECTION-CORE, API-PUBLIC-HISTORY-REVIEW-METADATA-CORE, API-PUBLIC-HISTORY-PROJECTION-CORE, and API-PUBLIC-EVENT-LIST-CANDIDATE-CORE. Their scope, limitations and checks are recorded in the [L1 assignment](assignments/L1-FIXTURE-RUNNER-CORE.md), [lookup assignment](assignments/API-PUBLIC-LOOKUPS-CORE.md), [snapshot assignment](assignments/API-PUBLIC-SNAPSHOT-CORE.md), [projection service assignment](assignments/API-PUBLIC-PROJECTION-SERVICE-CORE.md), [geometry reader assignment](assignments/API-PUBLIC-GEOMETRY-READER-CORE.md), [detail projection assignment](assignments/API-PUBLIC-DETAIL-PROJECTION-CORE.md), [history metadata assignment](assignments/API-PUBLIC-HISTORY-REVIEW-METADATA-CORE.md), [history projection assignment](assignments/API-PUBLIC-HISTORY-PROJECTION-CORE.md), and [event-list reader assignment](assignments/API-PUBLIC-EVENT-LIST-CANDIDATE-CORE.md).
...
Root independently reviewed agent commits `f3d7e08` (`feat(worker): project reviewed public event history`) and `e2267e9` (`docs(API-PUBLIC-HISTORY-PROJECTION-CORE): record implementation handoff`) on `work/API-PUBLIC-HISTORY-PROJECTION-CORE`, then cherry-picked them to `main` as `7f54beb` and `55ccb23`. The service composes injected candidate-version and exact-version disclosure ports, validates each page envelope, sequence, cursor and published live record identity, fails closed on missing or incomplete approval coverage, and constructs only the existing `HistoryEntry` fields. `changed_at` is the exact version's `published_at`; cursor expiry remains null in this unconnected slice.

Root independently verified in WSL Ubuntu-26.04 using Node `v24.21.0`, npm `11.19.0`, and Git `2.53.0`: focused tests 11/11; full `npm test` exit 0 (web 22, Worker 183, DB 15/15 files, evaluation 12); `npm run typecheck`; `npm run build` (Vite production and Wrangler `4.137.0` dry-run); and `git diff --check` plus both commit checks. No dependency, route, DTO, database adapter, migration or runtime configuration changed. All records were authored fictional values; hosted Neon, real moderator decisions, source rights and runtime composition remain unverified. MOD-01 still owns authenticated review writes. See the [assignment and handoff](assignments/API-PUBLIC-HISTORY-PROJECTION-CORE.md).

### API-PUBLIC-SNAPSHOT-CORE — accepted

Root reviewed and fast-forwarded `work/API-PUBLIC-SNAPSHOT-CORE` to local `main` at handoff `cac9e2aedb9361f351e2cf379291503eac81f78f` (`6a355bc24f00bbdb37fba34520cb2176df03531a` implementation). The internal reader accesses only `public_event_versions` and `public_event_impacts`, enforces the current live dataset and exact event version, and detects result overflow rather than truncating. It exposes a bounded `found`/`missing` result to internal callers; it does not create a public response.

Root independently passed focused PGlite tests 6/6, `npm run db:test` (12/12 files, 92 tests), full WSL `npm test` (271/271), typecheck, Vite production build, Wrangler `4.137.0` dry-run, and `git diff --check`. Hosted Neon/PostgreSQL behavior remains unverified. No API route, DTO, runtime DB binding, migration, source, or external service changed. See the [assignment and root review](assignments/API-PUBLIC-SNAPSHOT-CORE.md).

### API-PUBLIC-PROJECTION-SERVICE-CORE — accepted

Root reviewed and fast-forwarded `work/API-PUBLIC-PROJECTION-SERVICE-CORE` to local `main` at handoff `eae394bf44bdb2d6e7116deef0437dedb327364b`. It composes the accepted snapshot and reviewed lookup readers through `projectPublicEvent`, validates requested/event/impact identities and versions before lookup, derives only exact bounded event/claim/impact scope names and claim-support attribution keys, rejects any lookup row containing a public excerpt, and returns only the existing public `EventView` or missing. Root independently passed focused 38/38, full WSL suite 309/309 (web 22, Worker 183, DB 92 across 12 files, evaluation 12), typecheck, build, and `git diff --check`. It adds no route, DTO, database adapter/runtime binding, migration, or live data. Hosted Neon and HTTP integration remain unverified. See [assignment and root acceptance](assignments/API-PUBLIC-PROJECTION-SERVICE-CORE.md).

### API-PUBLIC-GEOMETRY-READER-CORE — accepted

Root reviewed and fast-forwarded `work/API-PUBLIC-GEOMETRY-READER-CORE` to local `main` at handoff `8129dd7b2bb135e94a7d02579f0df88b07624df3` (`04a43b2b9ce1866fe2953c2d4a8e76df7a689fe8` implementation). Migration 012 adds a security-barrier view that exposes only live geometry IDs referenced by the current published event or a matching published claim and normalized claim-geometry link. A narrow public-reader grant exposes only that view; the role cannot read geometry/evidence base tables. The injected repository accepts exact unique IDs up to 500, skips empty queries, validates identity and sorts results deterministically with redacted errors. L4 remains authoritative for full geometry schema, CRS84 and evidence-support validation.

Root independently passed the focused geometry test (8/8), full WSL suite (317/317: web 22, Worker 183, DB 100 across 13 files, evaluation 12), typecheck, Vite build, Wrangler `4.137.0` dry-run, and `git diff --check`. The task agent also passed `npm run db:test` (13/13 files) and the full suite. No dependencies were added. PGlite uses authored fictional records; hosted Neon grants, runtime binding and HTTP integration remain unverified. No public contract, route, or source-backed geometry was added. See the [assignment and handoff](assignments/API-PUBLIC-GEOMETRY-READER-CORE.md).

### API-PUBLIC-DETAIL-PROJECTION-CORE — accepted

Root reviewed and fast-forwarded `work/API-PUBLIC-DETAIL-PROJECTION-CORE` to local `main` at handoff `58edcf1d07630011c67f679adad5b8896b1b19d9` (`efed7c22208c3740dff971eb9ec4febe07f3c98f` implementation). It adds an injected read-only Layer 4 service composing one current event snapshot and exact lookup results with only geometry IDs from event/claim scopes. It rejects mismatched or incomplete reader output, caps the unique geometry set at 500, skips the geometry port for empty references, and passes untrusted records to the existing `projectPublicEventDetail` validator. The accepted EventView service uses the extracted shared snapshot preparation without behavior change.

Root independently passed the focused test (32/32), observed all 317 full-suite tests passing (web 22, Worker 183, DB 100 across 13 files, evaluation 12), `npm run typecheck`, `npm run build` (Vite production and Wrangler dry-run), and WSL `git diff --check`. The agent also passed those checks. The new focused test is run separately because the existing Worker script enumerates paths and package-script edits were outside scope. No migration, dependency, public contract, route, Worker binding or source was added. The test data is authored fiction; hosted Neon, source rights, runtime binding and live geometry remain unverified. See the [assignment and handoff](assignments/API-PUBLIC-DETAIL-PROJECTION-CORE.md).

### API-PUBLIC-HISTORY-READER-CORE — accepted

Root fast-forwarded the task branch to `main` at handoff `9832cf2` (`9401191` implementation). Migration 013 adds the security-barrier view and bounded keyset reader for published versions only while an event remains currently public. It returns untrusted internal schema 2.0 records and keeps all history hidden after latest withdrawal. Root independently passed full WSL `npm test` (exit 0: web 22, Worker 183, DB 14 files, evaluation 12), typecheck, build, and diff checks; the agent passed focused tests 7/7 and `npm run db:test` 14/14. No public summary/label projection, route, Worker database binding, source data or hosted Neon verification was added. See [assignment and handoff](assignments/API-PUBLIC-HISTORY-READER-CORE.md).

### API-PUBLIC-HISTORY-REVIEW-METADATA-CORE — accepted

Root reviewed and fast-forwarded `work/API-PUBLIC-HISTORY-REVIEW-METADATA-CORE` to local `main` at handoff `8187d30` (`9b2ab2a` implementation); root added test commit `1dc3218` to exercise the valid `+07:30` offset branch and reject an invalid offset. Migration 014 adds append-only decisions bound to exact live published versions and a security-barrier view that exposes only latest approved metadata for currently public history candidates. The injected repository uses a bounded parameterized keyset, validates exact event/version identities, keeps missing approvals visible as coverage gaps, and returns redacted failures without selecting event `record_json`. Newer held/revoked reviews suppress earlier approvals; latest event withdrawal hides all history.

Root independently passed the focused PGlite test (7/7), `npm run db:test` (15/15 isolated files), full `npm test` (web 22, Worker 183, DB 15 files, evaluation 12), `npm run typecheck`, `npm run build` (Vite production and Wrangler `4.137.0` dry-run), and WSL `git diff --check`. Verification used WSL Ubuntu-26.04 with Node `v24.21.0`, npm `11.19.0`, and Git `2.53.0`. No dependency or runtime configuration changed. Fixtures are authored synthetic data; hosted Neon behavior and real moderator decisions remain unverified. No public projection, DTO, route, Worker database binding or authenticated writer was added. MOD-01 owns authenticated review writes. See the [assignment and handoff](assignments/API-PUBLIC-HISTORY-REVIEW-METADATA-CORE.md).

### API-PUBLIC-LOOKUPS-CORE — accepted

Root fast-forwarded `work/API-PUBLIC-LOOKUPS-CORE` to `main` at handoff `0f5e7f2463b424857a552df880da59089e16e05c` (`535efebd52045856441a2297b24678ae0e250f75` implementation). The migration adds append-only scope-name and exact-live-support attribution review snapshots, safe latest-approved views, and reader-only view grants. The bounded repository uses parameterized exact keys, omits missing/held/revoked rows, and emits UTC RFC 3339 timestamps. Root's separate migration-test correction is `b69cda0`; no public route, approval writer, real source data, rights decision, or runtime DB connection was added.

Root independently passed focused lookup tests 6/6, all DB tests in 11/11 isolated files, full WSL `npm test` 265/265 (web 22, Worker 145, DB 86, evaluation 12), typecheck, Vite production build, Wrangler `4.137.0` dry-run, and `git diff --check`. PGlite does not verify hosted Neon behavior. Real approval records remain gated on MOD-01 and source/data permissions. See the [assignment and review](assignments/API-PUBLIC-LOOKUPS-CORE.md).

### UI-01-FEED-FILTERS-CORE — accepted

Root reviewed and fast-forwarded `work/UI-01-FEED-FILTERS-CORE` to local `main` at handoff `76aba84577a1be1a0bb4ed5e678f572f31779f6a` (`docs(UI-01): record feed filter handoff`), following implementation commit `3d00b29c6fa144e2f7304da4af245c60d253562b` (`feat(UI-01): add loaded-page feed filters`). It adds accessible category, lifecycle, freshness, and text-search controls over already-loaded API records, with AND semantics, matching/loaded counts, clear-all, selection reset, and distinct filtered-empty, API-empty, loading, and unavailable states. Empty filtered results explicitly do not establish safety; demo and evidence/time labels remain unchanged.

Root independently passed focused UI tests 9/9, full `npm test` 249/249 (web 22, Worker 137, DB 78, casebook 12), typecheck, Vite production build, Wrangler dry-run, and WSL diff checks. Screenshot review was not available because no preinstalled WSL headless browser or Playwright/Puppeteer package was present. No dependency, API/DTO, migration, configuration, external service, or live source changed. See [assignment and review](assignments/UI-01-FEED-FILTERS-CORE.md).
...
- API-PROJECT-CORE and API-GEOMETRY-CORE are accepted as pure Layer 4 allowlist projectors for `EventView`, `EventDetail`, and GeoJSON. Existing database `public_event_*` views still expose full internal schema 2.0 `record_json`; no view output can be serialized directly as these public DTOs. ADR-012 records the boundary. API-DETAIL-HISTORY-ROUTES-CORE adds demo-only detail/history handlers, and API-GEOJSON-ROUTE-CORE adds the empty synthetic GeoJSON route with ADR-018 bbox validation. API-01 still needs public-read database/role composition and live spatial filtering.
