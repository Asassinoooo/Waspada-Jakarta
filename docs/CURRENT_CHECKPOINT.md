...
**Reason:** Root accepted `API-PUBLIC-HISTORY-PROJECTION-CORE` on `main` at `55ccb23` (`7f54beb` implementation), preserving agent commits `f3d7e08` and `e2267e9`. The injected Layer 4 service joins bounded current-public event versions with exact moderator-reviewed metadata, requires complete review coverage, and returns only the existing five-field `HistoryEntry` allowlist. `changed_at` comes from the version's `published_at`; no reviewer IDs or raw event records escape. Root independently passed focused projection tests 11/11, full WSL `npm test` (web 22, Worker 183, DB 15 files, evaluation 12), typecheck, production build/Wrangler dry-run, and diff/commit checks. This synthetic-only service adds no route, runtime binding, review writer, or real approval. MOD-01 and source/data rights still gate authenticated writes and live history output. The GeoJSON demo route remains empty because no fixture has source-supported geometry.
...
The latest accepted task is API-PUBLIC-HISTORY-PROJECTION-CORE, integrated at root handoff commit `55ccb23` (implementation `7f54beb`; agent commits `f3d7e08`, `e2267e9`). The new injected Layer 4 service pairs bounded history candidates with exact-version approved disclosures, verifies matching pages/cursors and complete review coverage, and emits only the existing `HistoryPage` allowlist. Root independently passed focused tests 11/11, full WSL `npm test` (web 22, Worker 183, DB 15 files, evaluation 12), typecheck, production build/Wrangler dry-run, and diff/commit checks. Tests use authored fictional values. No route, runtime binding, database adapter, real review record, or source-rights decision was added; hosted Neon and live behavior remain unverified. Migration 014 and ADR-020 still require moderator-reviewed labels/summaries and hide withdrawn versions and all history after latest withdrawal. MOD-01 still owns authenticated review writes. API-01's next missing foundation is a bounded event-list reader; resolve cursor stability and preserve existing filters before assignment. UI-01 filters and UI-02 preferences remain accepted with no synthetic relevance matching.
