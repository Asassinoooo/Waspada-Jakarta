...
**Reason:** Root accepted API-PUBLIC-DETAIL-PROJECTION-CORE on local `main` at handoff `58edcf1` (`efed7c2` implementation), composing the accepted snapshot, reviewed lookups and exact geometry reader through the strict L4 EventDetail projector. Root's independent WSL tests, typecheck, build and diff checks passed. Root assigned API-PUBLIC-HISTORY-READER-CORE for a bounded read of published versions belonging to currently public events. Source rights, human-adjudicated labels, model/provider selection, moderator authorization, hosted Neon behavior, and Worker runtime integration remain gated. The GeoJSON demo route remains empty because no fixture has source-supported geometry.
**Repository:** `D:\Projects\RPL`
**Remote:** `origin` → `https://github.com/Asassinoooo/Waspada-Jakarta.git`

**Next assigned task:** `API-PUBLIC-HISTORY-READER-CORE`, on its dedicated branch/worktree from the pushed assignment commit. It will expose a security-barrier view and bounded keyset reader for published versions only when the event remains currently public. The reader returns untrusted internal records; public history summaries, retraction visibility, HTTP routes and runtime wiring remain out of scope.

## Repository state

The latest accepted task is API-PUBLIC-DETAIL-PROJECTION-CORE, fast-forwarded to local `main` at handoff `58edcf1` (`efed7c2` implementation). Its read-only service validates the event snapshot and exact reviewed lookup rows, derives only deduplicated event/claim geometry IDs, enforces the 500-geometry cap and skips empty geometry reads. It passes untrusted geometry records through the existing strict `EventDetail` projector, which enforces schema, CRS84 and exact claim/evidence support. Root independently passed focused tests 32/32, the full WSL suite 317/317 (web 22, Worker 183, DB 100 across 13 files, evaluation 12), typecheck, build, and diff checks. The focused test runs separately because the assigned path boundary excludes Worker package-script edits. No route, contract or Worker database binding was added. Hosted Neon and source-backed geometry remain unverified. API-PUBLIC-GEOMETRY-READER-CORE and the previous snapshot/lookup/EventView projection foundations are accepted; UI-01 filters and UI-02 preferences remain accepted with no synthetic relevance matching.
...
### API-PUBLIC-HISTORY-READER-CORE — assigned

The next bounded database task adds a least-privilege view and keyset reader for published versions of an event only while its current version remains public. It returns bounded internal schema 2.0 records for later L4 history projection; it does not determine public change summaries or make withdrawn events public. Authored PGlite fiction only; no API contract, route, runtime binding, or source acquisition. See [assignment](assignments/API-PUBLIC-HISTORY-READER-CORE.md).
