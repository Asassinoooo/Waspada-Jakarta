...
**Reason:** Root accepted API-GEOJSON-ROUTE-CORE on local `main` at handoff `29aa6b4` (implementation `84b4607`) after independent review and WSL verification. The synthetic `GET /api/v1/events.geojson` route validates the ADR-018 query envelope and returns an empty FeatureCollection because fixtures contain no source-supported geometry. UI-02-LOCAL-PREFERENCES-CORE remains accepted at `b469a02` (`be4c34d`); its browser-local interest editor has no briefing/update matching. The application query envelope is documented in ADR-018 and OpenAPI; it is not an administrative boundary or warning zone. API-01 still needs a database-backed public reader and live spatial filtering. The previous OpenAPI edit was restored before the separately documented bbox clarification. Source/data rights, human labels, model/provider selection, moderator authorization, hosted Neon behavior, and complete runtime integration remain gated.
