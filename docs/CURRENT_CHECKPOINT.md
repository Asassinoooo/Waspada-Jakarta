...
**Reason:** Root accepted API-PUBLIC-SNAPSHOT-CORE on local `main` at handoff `cac9e2a` (`6a355bc` implementation), adding a bounded reader over the existing public event and impact views. Independent WSL checks pass. Root assigned API-PUBLIC-PROJECTION-SERVICE-CORE as the next L4 composition task. Source rights, human-adjudicated labels, model/provider selection, moderator authorization, hosted Neon behavior, and Worker runtime integration remain gated. The GeoJSON demo route remains empty because no fixture has source-supported geometry.
**Repository:** `D:\Projects\RPL`
**Remote:** `origin` → `https://github.com/Asassinoooo/Waspada-Jakarta.git`

**Next assigned task:** `API-PUBLIC-PROJECTION-SERVICE-CORE`, on its dedicated branch/worktree from the pushed assignment commit. It composes the bounded snapshot and reviewed lookup ports through the existing strict L4 projector and returns only the current public `EventView`; it adds no route, public DTO, or runtime database wiring.

## Repository state

The latest accepted task is API-PUBLIC-SNAPSHOT-CORE at handoff `cac9e2a` (`6a355bc` implementation), fast-forwarded to local `main`. Root independently passed the focused test 6/6, DB suite 12/12 files (92 tests), full WSL `npm test` 271/271, typecheck, build, and WSL diff checks. The reader uses only existing safe public views; no route, database runtime binding, or live source was added. Hosted Neon behavior remains unverified. API-PUBLIC-LOOKUPS-CORE and the root test correction `b69cda0` remain accepted. UI-01 filters and UI-02 preferences remain accepted, with no synthetic relevance matching.
...
The accepted work list now includes L1-FIXTURE-RUNNER-CORE, API-PUBLIC-LOOKUPS-CORE, and API-PUBLIC-SNAPSHOT-CORE. Their scope, limitations, and independent checks are recorded in the [L1 assignment](assignments/L1-FIXTURE-RUNNER-CORE.md), [lookup assignment](assignments/API-PUBLIC-LOOKUPS-CORE.md), and [snapshot assignment](assignments/API-PUBLIC-SNAPSHOT-CORE.md).
...
### API-PUBLIC-PROJECTION-SERVICE-CORE — assigned

Root assigned a narrow Layer 4 service to compose the accepted snapshot and reviewed lookup readers through `projectPublicEvent`. It derives only exact bounded scope and claim-support keys, fails closed on missing or ambiguous lookup records, and returns only the existing public `EventView` or a missing result. The task adds no route, DTO, database adapter/runtime binding, migration, or live data. See [assignment](assignments/API-PUBLIC-PROJECTION-SERVICE-CORE.md).
