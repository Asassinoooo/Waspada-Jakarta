...
**Reason:** Root accepted API-PUBLIC-PROJECTION-SERVICE-CORE on local `main` at handoff `eae394b` (`3adcc01` implementation, `58727dd` excerpt guard), composing the snapshot and reviewed lookups into the strict public EventView. Independent WSL checks pass. Root assigned API-PUBLIC-GEOMETRY-READER-CORE as the next bounded database slice. Source rights, human-adjudicated labels, model/provider selection, moderator authorization, hosted Neon behavior, and Worker runtime integration remain gated. The GeoJSON demo route remains empty because no fixture has source-supported geometry.
**Repository:** `D:\Projects\RPL`
**Remote:** `origin` → `https://github.com/Asassinoooo/Waspada-Jakarta.git`

**Next assigned task:** `API-PUBLIC-GEOMETRY-READER-CORE`, on its dedicated branch/worktree from the pushed assignment commit. It will expose only geometry rows referenced by current published live event/claim scopes through a least-privilege database view and bounded exact-ID reader. It adds no route, L4 composition, or runtime database wiring.

## Repository state

The latest accepted task is API-PUBLIC-PROJECTION-SERVICE-CORE at handoff `eae394b` (`3adcc01` implementation and `58727dd` excerpt guard), fast-forwarded to local `main`. Root independently passed its focused test 38/38, the full WSL suite 309/309 (web 22, Worker 183, DB 92 across 12 files, evaluation 12), typecheck, build, and WSL diff checks. It verifies requested event and impact identity/version before lookups, limits lookups to the referenced event/claim/impact scopes and claim-support attributions, rejects excerpt-bearing rows, and returns only the strict public EventView or missing. No route or Worker runtime database binding was added. Hosted Neon behavior remains unverified. API-PUBLIC-LOOKUPS-CORE and the root test correction `b69cda0` remain accepted. UI-01 filters and UI-02 preferences remain accepted, with no synthetic relevance matching.
...
The accepted work list now includes L1-FIXTURE-RUNNER-CORE, API-PUBLIC-LOOKUPS-CORE, API-PUBLIC-SNAPSHOT-CORE, and API-PUBLIC-PROJECTION-SERVICE-CORE. Their scope, limitations, and independent checks are recorded in the [L1 assignment](assignments/L1-FIXTURE-RUNNER-CORE.md), [lookup assignment](assignments/API-PUBLIC-LOOKUPS-CORE.md), [snapshot assignment](assignments/API-PUBLIC-SNAPSHOT-CORE.md), and [projection service assignment](assignments/API-PUBLIC-PROJECTION-SERVICE-CORE.md).
...
### API-PUBLIC-GEOMETRY-READER-CORE — assigned

The next bounded database task adds a least-privilege view restricted to geometry referenced by the current published live event and claim scopes, plus a parameterized exact-ID repository with a strict request cap. Authored PGlite fixtures test isolation from historical, synthetic, unpublished, withdrawn, and unreferenced geometry. The reader is an internal input to later L4 composition; no HTTP route, runtime binding, geometry publication, or source-backed fact is added. See [assignment](assignments/API-PUBLIC-GEOMETRY-READER-CORE.md).
