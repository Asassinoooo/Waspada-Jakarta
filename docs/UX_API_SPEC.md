...
**Status:** SPEC-03 design baseline accepted on 24 September 2026. The local Worker serves synthetic `GET /api/v1/context`, `/api/v1/events`, `/api/v1/events/{event_id}`, `/api/v1/events/{event_id}/history`, and `/api/v1/events.geojson`. API-PROJECT-CORE and API-GEOMETRY-CORE provide pure Layer 4 allowlists for `EventView`, `EventDetail`, and GeoJSON. Detail/history use fictional demo fixtures; GeoJSON validates the requested viewport and returns an empty collection because those fixtures have no source-supported geometry. None of these routes reads from the database. A live public reader, the broader public and moderator flows, and live spatial filtering remain unimplemented. The optional GeoJSON `bbox` uses the application envelope in [ADR-018](decisions/ADR-018-jakarta-geojson-query-envelope.md); it is a query bound, not an official boundary or event warning area.
