...
| [ADR-018](ADR-018-jakarta-geojson-query-envelope.md) | Public GeoJSON application query envelope | Inclusive CRS84 envelope `[106.32,-6.40,106.98,-5.16]`, rounded outward from the 2025–2029 Jakarta regional-plan extent; query limit only, not an administrative boundary or danger geometry | Accepted for API-GEOJSON-ROUTE-CORE |
| [ADR-020](ADR-020-public-event-history-disclosure.md) | Public event-history disclosure | Each public version requires a moderator-reviewed change label and summary for its exact event/version; withdrawn versions and all history after latest withdrawal remain hidden | Accepted for API-PUBLIC-HISTORY-REVIEW-METADATA-CORE |
