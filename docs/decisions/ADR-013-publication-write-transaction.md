...
The write operation can fail atomically and safely replay within the local database design. It still does not establish source rights, factual support, moderator identity, hosted PostgreSQL concurrency behavior, or transactional integration with Cloudflare Workers. The database role and transaction port need provider-specific wiring and review before deployment. Raw `record_json` remains internal and must pass through ADR-012's public projection before serialization.
