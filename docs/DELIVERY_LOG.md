...
The SDP permits Layer 4 policy foundations to begin against reviewed fixtures before the full M5 integration gate. Root split out [PUB-POLICY-CORE](assignments/PUB-POLICY-CORE.md), a pure deterministic assessment of explicit moderator-reviewed L2 proposals against exact retrieved evidence, source/revision/remit/freshness state, supported geometry, and event-version concurrency. The policy returns per-claim dispositions and reason codes only. Persistence, authentication, automatic publication, learned thresholds, and factual-quality claims stay outside the task; no human labels are fabricated. It runs in a separate Worker policy worktree alongside the RAG-ACCESS-01 database privilege work.
