...
Schema validity is necessary but never sufficient for publication. L4 must reject or hold records with failed semantic or authorization checks; L5 records the reason and propagation outcome without silently changing policy. API-PROJECT-CORE implements a runtime-validated public whitelist for consumed schema 2.0 Event/Impact fields. It resolves names, explicitly public-approved source attributions and exact impact versions before constructing `EventView`; the database read and HTTP route remain unconnected.
