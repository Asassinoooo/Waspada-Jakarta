...
| `InvestigationRequest` / `InvestigationCheckpoint` | L3; gap-specific work and durable stop/resume state | A new, unmatched candidate has both event ID and event version null. A matched case has both populated. Resume retains the checkpoint's event pair and budget values. |
...
- A new investigation ledger starts from configured `limits`; `consumed` counts failed and successful actions, and `reserved` records preflight work not yet reconciled. Checkpoints preserve all three objects plus the budget-policy version. Remaining allowance is derived. Service validation prevents `consumed + reserved` exceeding a limit, double reservation, limit edits, or resetting counters on resume.
...
Schema validity is necessary but never sufficient for publication. L4 must reject or hold records with failed semantic or authorization checks; L5 records the reason and propagation outcome without silently changing policy. API-PROJECT-CORE implements a runtime-validated public whitelist for consumed schema 2.0 Event/Impact fields. It resolves names, explicitly public-approved source attributions and exact impact versions before constructing `EventView`; the database read and HTTP route remain unconnected. GEO-STORE-CORE adds a local L1 writer that validates a supplied schema 2.0 Geometry and resolves its exact persisted supporting spans before storing the shape and links atomically. Synthetic PGlite checks establish storage invariants only; they do not verify source rights or that a support relation proves the geometry's meaning.
