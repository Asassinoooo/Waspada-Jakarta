...
- Limitations and impact: Internal candidate reader only. It adds no route, public projection/DTO, OpenAPI/filter semantics, Worker/runtime binding, database connection, source access, real data, dependency, migration, or configuration change. Pagination is not a historical snapshot: returned records reflect the current published version, and current withdrawals disappear. Root review and integration remain outstanding; there are no unresolved design decisions for this assignment.
