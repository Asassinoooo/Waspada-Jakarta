...
- **Input-budget implementation commit:** `35f049b98df994ccd6628b2be5855849792c88f7` — `fix(L3-SINGLE-STEP-EXECUTOR-CORE): bound aggregate action input`
- **Handoff commits:** `cb2e84427841bee74bfb45c2bc7d0a102d16542c` — `docs(L3-SINGLE-STEP-EXECUTOR-CORE): record implementation handoff`, and `f796161759274feaa0e1576376182a1b6c23f0f0` — `docs(L3-SINGLE-STEP-EXECUTOR-CORE): update ledger replay handoff`; this document is updated in a separate handoff commit after the input-budget change.

## Behavior implemented

`createSingleStepExecutor` accepts one closed proposal with dataset, investigation ID, expected checkpoint version, caller-supplied reservation ID and timestamp, exact action name, and action-specific input. It checks action names and registry entries strictly, clones bounded JSON input before parsing, and returns closed denial or review results for malformed, unregistered, disabled, or action-invalid proposals before any ledger call. Input strings are limited to 16,384 UTF-16 code units each, their aggregate with plain-object keys is limited to 32,768 code units, and each object key is limited to 128 code units.
...
- From `apps/worker`, `/mnt/d/Projects/RPL/node_modules/.bin/tsx --test test/l3-single-step-executor.test.ts` — passed 16/16.
- `npm test` — passed 212/212 total: web 5, Worker 117, DB 78 across 10 isolated test files, and evaluation casebook 12.
