...
Acceptance is limited to local repository and adapter behavior under the fixture executor. The owner-level RAG-CORE tests alone did not establish restricted role access; [RAG-ACCESS-01](RAG-ACCESS-01.md) now adds a separate least-privilege role and proves the real retrieval SQL under `SET ROLE` for non-semantic, semantic, geometry, and combined paths. This is local synthetic PGlite evidence, not a hosted role/deployment test. `rowsExamined` bounds evidence-reference rows evaluated by the application; it is not a database physical-scan or latency guarantee. PGlite and synthetic vectors do not establish Neon behavior, retrieval quality, or evidence sufficiency. Worker/Neon connection wiring remains later integration work.
