...
Status: target architecture with partial local implementation. The repository contains a React/TypeScript demo shell, modular Worker read API, typed L2 model contracts, a synthetic GeoJSON fixture parser, deterministic L1 text preparation and chunk persistence, DATA-01's local PostgreSQL foundation, and GEO-STORE-CORE's typed transactional L1 geometry writer. The geometry writer validates CRS84 shape and exact persisted evidence links without geocoding or spatial expansion; source meaning and hosted Postgres behavior remain unverified. L1-WRITE-IDEMPOTENCY-CORE adds conflict-safe create-or-verify for report revisions and database uniqueness for evidence-reference identity, with no changes to schema 2.0 or L1 grants; hosted concurrency remains unverified. L1-FIXTURE-PIPE-CORE composes the accepted queue, parser, text, revision, evidence, chunk and geometry ports for already-leased synthetic moderator submissions. It uses an exact in-memory catalog and explicit manifest mappings, never fetches or infers event/geometry facts, and completes only after all writes; it is not connected to a production Worker route or scheduler. DB-TEST-RUNNER-ISOLATION runs each DB test file in its own sequential Node process; L1-EVIDENCE-RELATION-ALIGN-CORE now preserves all four schema 2.0 evidence relations in local storage and retrieval without changing L4 meaning. Root independently passed the 9-file DB suite (70 tests) and full 149-test workspace suite, typecheck and build. The earlier silent aggregate exit remains unexplained, and recurring child failures remain fail-visible without retry. RAG-CORE adds a read-only hybrid evidence retrieval repository and thin Worker adapter; it returns evidence candidates only and does not assess sufficiency, create proposals or start L3. RAG-ACCESS-01 adds and locally tests a dedicated least-privilege L2 reader role; Worker/database service-role wiring and hosted Neon behavior remain unverified. PUB-POLICY-CORE adds a pure L4 manual publication assessment, API-PROJECT-CORE adds the runtime-validated public whitelist projection, and PUB-WRITE-CORE adds a local transaction for approved event/impact writes with version checks, idempotency, audit and a minimal outbox. The publication writer checks the configured namespace and preserves evidence-backed geometry. It remains unexposed: MOD-01 authentication is absent, source rights remain pending, PGlite does not verify Neon concurrency, and the shared L4 role retains pre-existing source-policy, trace/audit and queue privileges. A narrower runtime role is required before HTTP publication wiring. The five layers describe logical responsibilities, not five microservices. The current physical target is React/TypeScript assets and a modular TypeScript API on Cloudflare Workers, bounded Cloudflare Workflows for asynchronous execution, and Neon Free Postgres with PostGIS/pgvector. Free-tier limits and failure behavior are specified in [ADR-010](docs/decisions/ADR-010-cloudflare-neon-free.md).
...
Suggested module boundaries are ingestion, preprocessing and knowledge_store (L1); model_adapters, retrieval and grounding (L2); investigation and tool_executor (L3); publication_policy, moderation, public_api and frontend (L4); and evaluation and telemetry (L5). The repository now has the web and Worker shells, separate `apps/db` persistence foundation, RAG retrieval repository, and pure L4 publication-policy kernel. Runtime retrieval, transactional event persistence, moderation authorization, most orchestration, and public APIs remain planned. Deploy the application modules in a TypeScript Worker with static frontend assets rather than a separately billed server. Keep scheduled acquisition, extraction and investigation as distinct bounded workflows; apply database roles and service boundaries so model/ingestion code cannot write public event versions. WSL hosts local development and tests.
