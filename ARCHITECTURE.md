...
The canonical schema 2.0 `EvidenceRef` and L2 contracts include `updates`, but the current database constraint and DB relation type do not. [ADR-016](docs/decisions/ADR-016-evidence-reference-relation-alignment.md) selects a narrow forward migration and typed-storage correction so all four contract values round-trip without changing their meaning. `updates` remains distinct from claim support, and the L4 publication policy stays fail-closed. After this prerequisite, [ADR-015](docs/decisions/ADR-015-l2-grounding-context-persistence.md) plans immutable persistence of the canonical refs-only `GroundingContext` and its normalized links; the expanded retrieval/reasoning payload and excerpt text are not part of that stored record.
