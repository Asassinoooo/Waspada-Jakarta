...
The local implementation now covers synthetic GeoJSON parsing, deterministic text preparation and chunk metadata persistence, plus a transactional geometry writer that stores caller-supplied CRS84 shape and persisted supporting evidence links under narrowly scoped L1 column reads. Report-revision writes still use plain inserts and evidence-reference writes still allocate a new identity on retry; the next local slice adds exact create-or-verify semantics before queue-to-fixture composition. The implementation does not poll a source, persist live source content, infer geometry roles, derive hazard areas, or connect the Worker to a hosted database. Source-field mapping, rights, and per-source approval remain activation gates.
