...
**Status:** Planning baselines and synthetic local implementation slices through API-GEOJSON-ROUTE-CORE, UI-02-LOCAL-PREFERENCES-CORE and UI-01-FEED-FILTERS-CORE are accepted. The GeoJSON demo route validates the recorded ADR-018 application query envelope and returns an empty collection because checked-in demo fixtures contain no source-supported geometry. The feed filters operate only on already-loaded synthetic demo records; they state the loaded-page scope and that no match does not imply safety. Root independently passed the focused task tests, full workspace suite (249/249), typecheck, Vite production build, Wrangler dry-run and WSL diff checks. Screenshot review for the filter slice was unavailable because WSL has no installed headless browser. The bounded synthetic queue-to-fixture runner is now assigned to compose the accepted L1 queue and processor; it does not enable network acquisition or Cron. Personal briefings and updates remain unimplemented, as do the database-backed public reader, live spatial filtering and live safety data. Source/data rights, human-adjudicated evaluation, moderator authorization and free-tier hosted-service validation continue to gate live-data work. Telemetry sinks remain opt-in because Worker/database runtime composition is not implemented.
...
The database-backed public reader depends on reviewed scope-name and exact-span source-attribution lookups. ADR-019 separates those append-only disclosure approvals from event publication, source acquisition approval, lifecycle, freshness, and user relevance. API-PUBLIC-LOOKUPS-CORE establishes only the local schema/read-view foundation with authored synthetic tests; no real approval records, excerpts, API route, or source-rights claim are created until MOD-01 and data permissions are in place. API-PUBLIC-SNAPSHOT-CORE is accepted as a bounded reader over the existing safe views, tested with authored synthetic rows. API-PUBLIC-PROJECTION-SERVICE-CORE is accepted as a bounded Layer 4 composition service: it binds request/event/impact identities, derives exact scope and claim-support lookup keys, rejects excerpt-bearing lookup results, and returns only the existing strict public EventView or a missing result. Root independently passed focused 38/38, full suite 309/309, typecheck, build, and WSL diff checks. Hosted Neon behavior and Worker/database runtime wiring remain unverified. API-PUBLIC-GEOMETRY-READER-CORE is now assigned to add a least-privilege live-only database view and bounded exact-ID reader for geometry referenced by current published event/claim scopes; this internal reader adds no route or runtime wiring.
