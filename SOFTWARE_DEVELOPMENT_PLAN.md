...
**Status:** Planning baselines and synthetic/local implementation slices through API-PUBLIC-HISTORY-PROJECTION-CORE are accepted. The history foundation includes migration 013's bounded published-version reader, migration 014's append-only exact-version review metadata, and an injected Layer 4 projector that composes both into the unchanged `HistoryPage` allowlist. The projector requires complete reviewed coverage, derives `changed_at` only from that version's `published_at`, and never exposes reviewer IDs or raw records. Per ADR-020, moderator-reviewed labels and summaries are required; withdrawn versions and all history after a latest withdrawal stay hidden. Root independently passed focused projection tests (11/11), full WSL `npm test` (web 22, Worker 183, DB 15 files, evaluation 12), typecheck, build, and commit/diff checks. This remains a synthetic-only service foundation: authenticated review writes, public route/runtime wiring, live data, human labels, and hosted-provider validation remain gated on MOD-01 and source/data rights. The next assigned API-01 slice, API-PUBLIC-EVENT-LIST-CANDIDATE-CORE, is a bounded current-public event candidate reader ordered by initial publication time and event ID; public filters and route/runtime integration remain later work. The GeoJSON demo route still returns an empty collection because fixtures contain no source-supported geometry. Personal briefings and updates remain unimplemented; telemetry sinks remain opt-in because Worker/database runtime composition is not implemented.
...
API-PUBLIC-EVENT-LIST-CANDIDATE-CORE is assigned as the next DB-only API-01 foundation. It will page current-public live event candidates by descending initial version-1 publication time and ascending event ID, using a bounded internal keyset. The initial-publication anchor stays fixed when later event versions change; a current withdrawal still removes the event. This slice adds no query filters, L4 projection, route, DTO, or runtime wiring; those remain separate API-01 work.
