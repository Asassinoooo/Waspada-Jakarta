...
**Status:** Planning baselines and synthetic/local implementation slices through API-PUBLIC-HISTORY-READER-CORE are accepted. Migration 013 adds a least-privilege history view and bounded keyset reader for published versions only while an event remains a current public live event. The reader returns untrusted internal schema 2.0 records; it does not publish summaries or infer change types. Per ADR-020, each public history entry requires a moderator-reviewed label and summary tied to the exact event version; withdrawn versions and all history after a latest withdrawal stay hidden. Root independently passed the full WSL workspace suite (web 22, Worker 183, DB 14 files, evaluation 12), typecheck, build and diff checks. The accepted GeoJSON demo route still returns an empty collection because fixtures contain no source-supported geometry. No Worker database binding, live public response path, live spatial filtering or live safety data is connected. Source/data rights, human-adjudicated evaluation, moderator authorization and free-tier hosted-service validation continue to gate live-data work. Personal briefings and updates remain unimplemented; telemetry sinks remain opt-in because Worker/database runtime composition is not implemented.
