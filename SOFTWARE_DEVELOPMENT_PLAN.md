...
**Status:** Planning baselines and synthetic/local implementation slices through `API-PUBLIC-EVENT-LIST-CANDIDATE-CORE` are accepted. The public-history foundation includes migration 013's bounded published-version reader, migration 014's append-only exact-version review metadata, and a read-only Layer 4 projector for the unchanged `HistoryPage` allowlist. Moderator-reviewed labels and summaries remain required; withdrawn versions and all history after a latest withdrawal stay hidden. The accepted event-list foundation adds an injected bounded keyset reader over current-public live views, anchored by version 1's `published_at` descending and `event_id` ascending. Root independently passed the focused WSL suite after integration (2/2). The implementation commit passed `npm run db:test` (16 files), `npm test` (web 22, Worker 183, DB 16 files, evaluation 12), typecheck, build and diff checks; the later test-only follow-up passed focused checks (2/2). The next bounded task composes these candidates through the existing Layer 4 `EventView` service; public query filters, cursor serialization, routes and runtime wiring remain later work. Hosted Neon behavior, live records, authenticated review writes and source/data rights remain unverified or gated on MOD-01 and approved rights. The GeoJSON demo route still returns an empty collection because fixtures contain no source-supported geometry. Personal briefings and updates remain unimplemented; telemetry sinks remain opt-in because Worker/database runtime composition is not implemented.
...
Local project startup and verification use WSL Ubuntu-26.04. BOOT-01 provides the local React/Worker shell and DATA-01 provides a PostgreSQL migration/repository foundation with a synthetic PGlite test harness; neither provisions an external database or wires database access into the Worker. GEO-STORE-CORE adds a typed L1 PostGIS writer that validates and stores supplied CRS84 geometry with exact persisted supporting evidence links and narrow L1 reads; its synthetic tests do not establish semantic support or hosted behavior. L1-WRITE-IDEMPOTENCY-CORE adds create-or-verify semantics for immutable report revisions and a database-enforced identity for evidence references; migration 007 refuses ambiguous legacy duplicates without modifying rows. L1-FIXTURE-PIPE-CORE adds an injected exact-URL catalog processor for already-leased synthetic moderator submissions: it builds closed, unreviewed schema 2.0 revisions from explicit manifests, prepares text/chunks, stores only explicitly supported 2D geometry, and completes only after persistence. It does not fetch URLs or wire a live route or scheduler. DB-TEST-RUNNER-ISOLATION runs each DB test file in a separate sequential Node process; the earlier silent aggregate exit remains unexplained and the runner does not retry or hide recurrence. PGlite remains a local single-session check. API-PROJECT-CORE provides a strict L4 whitelist projection to the existing EventView, but does not connect database reads to HTTP. L2-CONTEXT-PERSIST-CORE provides a transactional local writer for canonical refs-only schema 2.0 contexts; L2-CONTEXT-BRIDGE-CORE validates the expanded reasoning request and passes only its canonical reference projection to the injected writer, returning evidence excerpts and provenance separately in memory. RAG-CONTEXT-ASSEMBLY-CORE rehydrates caller-selected exact Unicode spans under existing L2 read grants, preserves evidence provenance, timestamps, conflicts and caller-supplied sufficiency, then validates the schema 2.0 reasoning request; it does not invoke a model, L3, or an application route. L2-DIRECT-REASONING-CORE persists validated refs-only context before branching: insufficient context returns an investigation handoff without calling the reasoner, while sufficient context calls the injected adapter and passes its typed outcome through. Its safe telemetry contains only a closed outcome and duration; no real provider or L3 coordinator is configured. L3-INSUFFICIENT-CONTEXT-ENTRY-CORE creates or replays a bounded ledger case from exact persisted insufficient context, deriving only stable non-content question labels and routing ambiguous event matches or invalid question counts to review. L3-SINGLE-STEP-EXECUTOR-CORE validates a bounded proposal and registered action, lets the ledger resolve exact idempotent reservation replays before current case-state denials, invokes at most one handler only after an authorized start, and reconciles a bounded receipt. Its input is capped at 32,768 UTF-16 code units across values and plain-object keys, with a 128-code-unit key cap. It uses only injected synthetic handlers; no planner, action registration, Worker/database runtime wiring, live source, route or publication behavior is enabled. Root independently passed the 212-test WSL suite, typecheck, build and branch diff checks. No hosted database-session wiring, external account, database project, paid model, live connector or deployment has been created. Source/data rights and human adjudicated labels remain pending; EVAL-01 real case collection, source activation, provider benchmarking, and dependent RAG/publication/API work remain gated. Data retention is constrained by Neon Free's 0.5 GB/project cap: enforce a small demo corpus, monitor storage, pause new acquisition before the cap, and preserve current evidence/audit rows rather than silently evicting them. Until a free off-provider backup and deletion-replay procedure is implemented and rehearsed, use synthetic/historical demo data only.

L1-FIXTURE-RUNNER-CORE composes the scoped queue claim with the exact-catalog processor for at most one due synthetic moderator submission per invocation; SQL filters before lease and the caller supplies the transition timestamp. It adds no fetch, route, Cron trigger, runtime database connection, publication, or live-source behavior.
...
The database-backed public reader depends on reviewed scope-name and exact-span source-attribution lookups. ADR-019 separates those append-only disclosure approvals from event publication, source acquisition approval, lifecycle, freshness, and user relevance. API-PUBLIC-LOOKUPS-CORE establishes the local schema/read-view foundation with authored synthetic tests; no real approval records, excerpts, API route, or source-rights claim are created until MOD-01 and data permissions are in place. API-PUBLIC-SNAPSHOT-CORE is accepted as a bounded reader over the existing safe views, tested with authored synthetic rows. API-PUBLIC-PROJECTION-SERVICE-CORE composes those inputs through the strict public EventView allowlist. API-PUBLIC-GEOMETRY-READER-CORE is accepted with migration 012: its security-barrier view limits access to exact geometry IDs in current published live event/claim scopes, and its injected repository performs bounded parameterized reads. API-PUBLIC-DETAIL-PROJECTION-CORE is accepted as an injected read-only composition through `projectPublicEventDetail`; it validates exact snapshot/lookup/geometry identities and lets the existing L4 projector enforce public geometry and evidence support. Focused detail tests passed 32/32; full workspace tests passed 317/317, with typecheck, Vite build, Wrangler dry-run and WSL diff checks. Authored fixtures do not verify hosted Neon behavior. API-PUBLIC-HISTORY-READER-CORE is accepted with migration 013: a security-barrier view exposes only published versions belonging to currently public live events, and an injected repository reads exact event IDs through bounded version keysets. The internal record JSON remains untrusted input for L4. API-PUBLIC-HISTORY-REVIEW-METADATA-CORE is accepted with migration 014: append-only decisions bind to exact live published versions, and a security-barrier view exposes only the latest approved metadata for versions still visible through the history reader. Its repository preserves per-version approval gaps, validates bounded keyset pages and redacts errors. ADR-020 requires moderator-reviewed labels/summaries and hides withdrawn versions and all history after latest withdrawal. API-PUBLIC-HISTORY-PROJECTION-CORE is accepted as a read-only Layer 4 service over both ports. It validates both pages and exact version pairing, requires complete approval coverage, emits only the unchanged five-field `HistoryEntry`, and uses the event version's `published_at` as `changed_at`. Root independently passed focused tests 11/11, full WSL tests (web 22, Worker 183, DB 15 files, evaluation 12), typecheck, production build/Wrangler dry-run, and diff checks. All test records are authored fiction. No public history route, runtime binding, real approval, source-rights decision, or live spatial filtering was added; MOD-01 still owns authenticated review writes.
...
UI-01-FEED-FILTERS-CORE adds category, lifecycle, freshness, and text-search filters over the records already loaded by the demo API page. It exposes the matching and loaded counts, supports clearing all filters, resets map selection when the visible set changes, and states that filtering is not a complete incident census or a safety assessment. No API, contract, fixture, geometry, or live-data behavior changed.

`API-PUBLIC-EVENT-LIST-CANDIDATE-CORE` is accepted as a DB-only API-01 foundation. Its bounded internal keyset pages current-public live event candidates by descending initial version-1 publication time and ascending event ID. The initial-publication anchor stays fixed when later event versions change; a current withdrawal removes the event. The reader uses existing least-privilege public views and adds no query filters, L4 projection, route, DTO, migration or runtime wiring; those remain separate API-01 work.

`API-PUBLIC-EVENT-LIST-PROJECTION-CORE` is assigned as the next bounded Layer 4 composition. It will pass current-public candidate IDs through the existing strict `EventView` service, preserve candidate order and internal cursor progress, and cap projector concurrency. Query filters, URL parsing, public cursor encoding, route and Worker/database runtime wiring remain outside this task.

## 6. Data lifecycle and status model

The schema stores investigation requests and checkpoints; [ADR-014](docs/decisions/ADR-014-l3-investigation-ledger.md) defines the durable L3 persistence boundary. `L3-LEDGER-CORE` is accepted on `main` with same-candidate refreshed-context checkpoints, idempotent reservations and budget reconciliation. It does not invoke models/tools or grant publication authority. `DB-TEST-RUNNER-ISOLATION` is accepted on `main`; each DB test runs in a separate sequential Node process, and root independently passed the DB and full-workspace suites. The cause of the former silent aggregate exit and one transient silent migration-suite exit remains unknown; the runner does not retry or hide any recurrence. `L1-EVIDENCE-RELATION-ALIGN-CORE` adds migration 009 and aligns the local storage type with the existing schema 2.0 enum. It is a representation correction only; `updates` remains distinct and does not automatically count as publication support. [ADR-015](docs/decisions/ADR-015-l2-grounding-context-persistence.md) implements canonical refs-only context persistence. Accepted [L2-CONTEXT-BRIDGE-CORE](docs/assignments/L2-CONTEXT-BRIDGE-CORE.md) validates and projects reasoning requests into the injected refs-only writer while returning excerpts/provenance only in memory and preserving `sufficient` without interpretation.
...
The public and moderator projections are specified separately in [docs/UX_API_SPEC.md](docs/UX_API_SPEC.md) and [docs/api/openapi.yaml](docs/api/openapi.yaml). They deliberately omit internal model runs and raw evidence offsets from public responses. SPEC-02/03 are accepted as design contracts; OpenAPI-to-domain projection mapping is explicit in SPEC-03. API-PROJECT-CORE implements runtime validation and the public EventView allowlist. API-DETAIL-HISTORY-ROUTES-CORE and API-GEOJSON-ROUTE-CORE provide demo-only HTTP routes over synthetic fixtures; the GeoJSON route returns an empty collection because no source-supported demo geometry is available. API-PUBLIC-LOOKUPS-CORE, API-PUBLIC-SNAPSHOT-CORE, API-PUBLIC-PROJECTION-SERVICE-CORE and API-PUBLIC-GEOMETRY-READER-CORE provide local database/read foundations. API-PUBLIC-DETAIL-PROJECTION-CORE composes those inputs into the unchanged EventDetail contract. None is wired to a Worker connection or live public HTTP route. API-PUBLIC-HISTORY-READER-CORE and API-PUBLIC-HISTORY-REVIEW-METADATA-CORE provide bounded current-public version candidates and exact-version reviewed disclosures. API-PUBLIC-HISTORY-PROJECTION-CORE is accepted as a strict read-only Layer 4 composition into the existing `HistoryPage`: it validates exact live published record identity and timestamps, pairs bounded reader pages and cursors, requires complete approved review coverage, and emits only the five existing public fields. Its tests use authored fictional records; it adds no route, database adapter, or runtime binding. MOD-01 remains a prerequisite for authenticated review writes and any live public-history integration. PUB-WRITE-CORE implements the atomic manual write boundary with namespace, version, evidence and idempotency checks; it has no HTTP or authentication wiring. Before a Worker route uses it, MOD-01 must provide actor authorization and a dedicated runtime role must replace the current shared L4 role, which still carries source-policy, trace/audit and queue privileges.
