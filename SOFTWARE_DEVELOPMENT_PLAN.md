...
**Status:** Planning baselines and synthetic/local implementation slices through API-PUBLIC-DETAIL-PROJECTION-CORE are accepted. The geometry reader is accepted with migration 012 and a least-privilege current-published reference view. The detail composer now combines the accepted snapshot, reviewed lookup and geometry ports through the strict L4 EventDetail projector. Focused detail tests passed 32/32; the full workspace suite passed 317/317, along with typecheck, Vite production build, Wrangler dry-run and WSL diff checks. The accepted GeoJSON demo route still returns an empty collection because fixtures contain no source-supported geometry. No Worker database binding, live public response path, live spatial filtering or live safety data is connected. API-PUBLIC-HISTORY-READER-CORE is assigned to add a bounded least-privilege read of published versions for a currently public event; public history projection and retraction behavior remain separate decisions/tasks. Source/data rights, human-adjudicated evaluation, moderator authorization and free-tier hosted-service validation continue to gate live-data work. Personal briefings and updates remain unimplemented; telemetry sinks remain opt-in because Worker/database runtime composition is not implemented.
