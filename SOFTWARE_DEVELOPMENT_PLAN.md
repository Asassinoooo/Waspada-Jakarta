...
**Status:** Planning baselines and partial local implementations (BOOT-01, UI-00, PLATFORM-01, DATA-01, JOB-01, DATA-02-CORE, GEO-STORE-CORE, L1-WRITE-IDEMPOTENCY-CORE, L1-FIXTURE-PIPE-CORE, RAG-CORE, RAG-ACCESS-01, PUB-POLICY-CORE, PUB-WRITE-CORE and API-PROJECT-CORE) accepted; L3-LEDGER-CORE is assigned as a local prerequisite for the bounded coordinator; current aggregate local verification passes, while the historical DB-runner exit remains unexplained and unreproduced; source/model integration, Worker/database runtime wiring, evaluation and the complete prototype remain in progress
...
Local project startup and verification use WSL Ubuntu-26.04. BOOT-01 provides the local React/Worker shell and DATA-01 provides a PostgreSQL migration/repository foundation with a synthetic PGlite test harness; neither provisions an external database or wires database access into the Worker. GEO-STORE-CORE adds a typed L1 PostGIS writer that validates and stores supplied CRS84 geometry with exact persisted supporting evidence links and narrow L1 reads; its synthetic tests do not establish semantic support or hosted behavior. L1-WRITE-IDEMPOTENCY-CORE adds create-or-verify semantics for immutable report revisions and a database-enforced identity for evidence references; migration 007 refuses ambiguous legacy duplicates without modifying rows. L1-FIXTURE-PIPE-CORE adds an injected exact-URL catalog processor for already-leased synthetic moderator submissions: it builds closed, unreviewed schema 2.0 revisions from explicit manifests, prepares text/chunks, stores only explicitly supported 2D geometry, and completes only after persistence. It does not fetch URLs or wire a live route or scheduler. Its Worker checks, typecheck and build pass; all eight DB test files and both aggregate test commands passed in the current WSL verification. The historical aggregate exit 1 was not reproduced, and no runner change was made without a demonstrated cause. PGlite remains a local single-session check. API-PROJECT-CORE provides a strict L4 whitelist projection to the existing EventView, but does not connect database reads to HTTP. No external account, database project, paid model, live connector or deployment has been created. Data retention is constrained by Neon Free's 0.5 GB/project cap: enforce a small demo corpus, monitor storage, pause new acquisition before the cap, and preserve current evidence/audit rows rather than silently evicting them. Until a free off-provider backup and deletion-replay procedure is implemented and rehearsed, use synthetic/historical demo data only.

## 6. Data lifecycle and status model

The schema already stores investigation requests and checkpoints; [ADR-014](docs/decisions/ADR-014-l3-investigation-ledger.md) resolves the persistence boundary needed for a durable L3 coordinator. `L3-LEDGER-CORE` is assigned to implement same-candidate refreshed-context checkpoints, idempotent reservations and budget reconciliation locally, without invoking a model/tool or granting publication authority.
