...
**Status:** Planning baselines and synthetic/local implementation slices through API-PUBLIC-HISTORY-REVIEW-METADATA-CORE are accepted. Migration 014 stores append-only moderator-reviewed disclosure decisions for exact published live event versions and exposes only the latest approved metadata through a security-barrier view. The bounded reader preserves missing approval as an explicit coverage gap; it does not infer history labels or summaries. Per ADR-020, each public history entry needs a moderator-authored label and summary tied to its exact version; withdrawn versions and all history after a latest withdrawal stay hidden. Root independently passed focused metadata tests (7/7), `npm run db:test` (15/15 files), full WSL `npm test` (web 22, Worker 183, DB 15 files, evaluation 12), typecheck, build and diff checks. The next ready slice is a synthetic-only Layer 4 HistoryPage projection; authenticated writes, public route/runtime wiring, live data and hosted-provider validation remain gated on MOD-01 and source/data rights. The accepted GeoJSON demo route still returns an empty collection because fixtures contain no source-supported geometry. Personal briefings and updates remain unimplemented; telemetry sinks remain opt-in because Worker/database runtime composition is not implemented.
