...
**Status:** Planning baselines and partial local implementations (BOOT-01, UI-00, PLATFORM-01, DATA-01, JOB-01, DATA-02-CORE, RAG-CORE, RAG-ACCESS-01, PUB-POLICY-CORE, PUB-WRITE-CORE and API-PROJECT-CORE) accepted; source/model integration, Worker/database wiring, evaluation and the complete prototype remain in progress
...
Local project startup and verification use WSL Ubuntu-26.04. BOOT-01 provides the local React/Worker shell and DATA-01 provides a PostgreSQL migration/repository foundation with a synthetic PGlite test harness; neither provisions an external database or wires database access into the Worker. API-PROJECT-CORE now provides a strict L4 whitelist projection to the existing EventView, but does not connect database reads to HTTP. No external account, database project, paid model, live connector or deployment has been created. Data retention is constrained by Neon Free's 0.5 GB/project cap: enforce a small demo corpus, monitor storage, pause new acquisition before the cap, and preserve current evidence/audit rows rather than silently evicting them. Until a free off-provider backup and deletion-replay procedure is implemented and rehearsed, use synthetic/historical demo data only.
