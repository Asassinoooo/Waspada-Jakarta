...
**Status:** Planning baselines and synthetic local implementation slices through API-GEOMETRY-CORE are accepted. Root independently passed the focused geometry suite (8 tests), full workspace suite (220 tests), typecheck, Vite production build and Wrangler dry-run in WSL Ubuntu-26.04. The L4 geometry projection adds exact event/claim/support checks and bounded CRS84 output but has no database or HTTP wiring. Telemetry sinks remain opt-in because Worker/database runtime composition is not implemented. Source/data rights, human-adjudicated evaluation, model/provider selection, hosted service behavior and the complete prototype remain gated or in progress.
