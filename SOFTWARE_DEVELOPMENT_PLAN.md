...
Local project startup and verification use WSL Ubuntu-26.04. BOOT-01 provides the local React/Worker shell and DATA-01 provides a PostgreSQL migration/repository foundation with a synthetic PGlite test harness; neither provisions an external database or wires database access into the Worker. No external account, database project, paid model, live connector or deployment has been created. Data retention is constrained by Neon Free's 0.5 GB/project cap: enforce a small demo corpus, monitor storage, pause new acquisition before the cap, and preserve current evidence/audit rows rather than silently evicting them. Until a free off-provider backup and deletion-replay procedure is implemented and rehearsed, use synthetic/historical demo data only.
