...
**Status:** Planning baselines and synthetic/local implementation slices through API-PUBLIC-GEOMETRY-READER-CORE are accepted. Root has independently verified the geometry reader with focused tests, the full workspace suite (317/317), typecheck, Vite production build, Wrangler dry-run and WSL diff checks. Migration 012 exposes only geometries referenced by current published live event/claim scopes through a least-privilege view and bounded exact-ID reader. The accepted GeoJSON demo route still returns an empty collection because fixtures contain no source-supported geometry. No Worker database binding, live public response path, live spatial filtering or live safety data is connected. API-PUBLIC-DETAIL-PROJECTION-CORE is assigned to compose the accepted snapshot, lookup and geometry readers through the existing strict L4 EventDetail projector; it adds no route or runtime binding. Source/data rights, human-adjudicated evaluation, moderator authorization and free-tier hosted-service validation continue to gate live-data work. Personal briefings and updates remain unimplemented; telemetry sinks remain opt-in because Worker/database runtime composition is not implemented.
