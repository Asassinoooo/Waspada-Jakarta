...
**Status:** Planning baselines and synthetic local implementation slices through API-GEOJSON-ROUTE-CORE and UI-02-LOCAL-PREFERENCES-CORE are accepted. The GeoJSON demo route validates the recorded ADR-018 application query envelope and returns an empty collection because the checked-in demo fixtures contain no source-supported geometry. Root independently passed the focused API tests, full workspace suite, typecheck, Vite production build, Wrangler dry-run and WSL diff checks for this slice. The preferences screen saves only in browser storage and makes no relevance or safety claims; personal briefings and updates remain unimplemented. There is still no database-backed public reader, spatial filtering over live published events, or live safety data. The next assigned slice adds category/lifecycle/freshness filters to already-loaded feed records without changing the API. Telemetry sinks remain opt-in because Worker/database runtime composition is not implemented. Source/data rights, human-adjudicated evaluation, model/provider selection, hosted service behavior and the complete prototype remain gated or in progress.
