...
The proposed stack is React/TypeScript and Leaflet (Layer 4), Python/FastAPI with strict schemas (Layer 4), PostgreSQL/PostGIS/pgvector (Layer 1), separate extraction/embedding/reasoning adapters and hybrid retrieval (Layer 2), a LangGraph coordinator (Layer 3), and evaluation/telemetry modules (Layer 5). Provider/model choices remain subject to measured Indonesian-language accuracy, latency and cost. The shared contracts distinguish ReportRevision, ExtractionResult, GroundingContext, InvestigationRequest, EventProposal and PublicationDecision. A model never supplies an authoritative publish flag. Schema validation checks structure; evidence assessments and moderator review address semantic support.
